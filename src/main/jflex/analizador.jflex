...
STRING = \"[^\"]*\"
REAL = {INTEIRO}\.{DIGITO}+
RESERVADOS = {LETRA}({LETRA}|{DIGITO})*|"and"|"array"|"begin"|"case"|"const"|"div"|"do"|"downto"|"else"|"end"|"file"|"for"|"function"|"goto"|"if"|"in"|"label"|"mod"|"nil"|"not"|"of"|"or"|"packed"|"procedure"|"program"|"record"|"repeat"|"set"|"then"|"to"|"type"|"until"|"var"|"while"|"with"
SIMBOLOS = ":="|">="|"<="|"<>"|"="|":"|"\+"|"-"|"/"|"*"|">"|"<"|","|";"|"."|"\(" | "\)"
LINHA = \r|\n|\r\n
TABULACAO = {LINHA} | [ \t\f]
...
public static void main(String[] argv) {
    if (argv.length == 0) {
      System.out.println("Usage : java Analizador_Lex [ --encoding <name> ] <inputfile(s)>");
    }
    else {
      int firstFilePos = 0;
      ...
      for (int i = firstFilePos; i < argv.length; i++) {
        Analizador_Lex scanner = null;
        java.io.FileInputStream stream = null;
        java.io.Reader reader = null;
        try {
          stream = new java.io.FileInputStream(argv[i]);
          reader = new java.io.InputStreamReader(stream, encodingName);
          scanner = new Analizador_Lex(reader);
